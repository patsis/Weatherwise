...
"""
   ...
   func fetchForecast(type: FetchWeatherEndPoint) async throws -> [WeatherForecast] {
      ...
      do {
         ...
         // check http status code: if failed, throw error
   ...
   func fromJSON(type: FetchWeatherEndPoint) throws -> [WeatherForecast] {
   ...
   func update() async throws {
      dailyForecast = try fromJSON(type: .daily)//try await fetchForecast(type: .daily)
      hourlyForecast = try fromJSON(type: .hourly)//try await fetchForecast(type: .hourly)
   }
